...
function ExploreCourses() {
  ...
  const heroPrimary = isAdmin
    ? {
        label: "Go to Manage Courses",
        action: () => navigate("/admin/courses"),
      }
    : { label: "View Skill Courses", action: () => navigate("/allcourses") };
    ...
    <div className="w-full min-h-[60vh] flex flex-col lg:flex-row items-center justify-center gap-8 px-6 py-10">
      <div className="w-full lg:w-[350px] flex flex-col items-start justify-center gap-2">
        ...
        <p className="text-[16px] text-gray-600">
          {isAdmin
            ? "Manage internal learning paths, lectures, and assignments."
            : "Learn assigned skills with internal courses, assessments, and verified badges."}
        </p>
        ...
        <button
          ...
          onClick={heroPrimary.action}
        >
          {heroPrimary.label} <SiViaplay className="w-6 h-6 text-[#38D2CA]" />
        </button>

        <div className="mt-6 text-sm text-gray-600">
          {isAdmin ? "Admin View" : "Employee/Manager View"}
        </div>
      </div>
      ...
      <div className="w-full lg:w-[720px] grid grid-cols-2 md:grid-cols-4 gap-10 place-items-center">
        {tracks.map((t, i) => (
          <button
            ...
            type="button"
            onClick={() => {
              // Keep it simple: navigate to role-appropriate page.
              // Course filtering can be added later when real category data exists.
              heroPrimary.action();
            }}
          ...
          >
            <div
              className={`w-[100px] h-[100px] bg-gradient-to-br ${t.bg} rounded-2xl flex items-center justify-center shadow-lg hover:shadow-2xl transition`}
            >
